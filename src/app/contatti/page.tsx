            ...
            >
              {status === 'sent' ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {status === 'error' && (
                    <p className="text-sm text-red-600">Si è verificato un errore. Riprova o scrivici a {siteConfig.email}</p>
                  )}
                  <button
                    type="submit"
                    disabled={status === 'sending'}
                    className="w-full bg-black text-white py-3 text-sm font-medium hover:bg-neutral-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {status === 'sending' ? (
                      <>
